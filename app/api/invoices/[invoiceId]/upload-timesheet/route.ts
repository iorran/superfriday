/**
 * Upload Timesheet API Route
 * Uploads a timesheet file to blob storage and associates it with an invoice
 */
...
export async function POST(
  ...
  try {
    ...
    // Generate filename in year.month format with timesheet suffix
    const year = invoice.year || new Date().getFullYear()
    const month = invoice.month || new Date().getMonth() + 1
    const fileExtension = file.name.split('.').pop() || 'pdf'
    const fileName = `${year}.${String(month).padStart(2, '0')}-timesheet.${fileExtension}`
    ...
    await uploadFile(fileKey, fileBuffer, file.type || 'application/octet-stream')

    // Check if timesheet file already exists for this invoice
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to upload timesheet'
    return NextResponse.json(
